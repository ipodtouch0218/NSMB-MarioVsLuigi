namespace Fusion.Editor {
  ...
  public class FusionRunnerVisibilityControlsWindow : EditorWindow {
    ...
    static Lazy<GUIContent> s_noVisibilityWarn = new Lazy<GUIContent>(() => {
      return new GUIContent(FusionEditorSkin.WarningIcon, $"Network Runner does not have a {nameof(RunnerEnableVisibility)} component, or has not been registered with {nameof(NetworkRunnerVisibilityExtensions.EnableVisibilityExtension)}");
    });
    ...
    private void OnGUI() {
      ...
      bool runnerWithoutVisibilityEnabledDetected = false;
      ...
      if (!Application.isPlaying) {
        ...
        while (enumerator.MoveNext()) {
          ...
          EditorGUILayout.BeginHorizontal();
          {
            ...
            var lclplayerid = lclplayer.IsRealPlayer ? "P" + lclplayer.PlayerId.ToString() : "--";
            ...
            toggleGuiContent.text = runner.IsSharedModeMasterClient ? $"{runnerName} [MC]" : runnerName;
            ...
            if (shortText == false) {
              ...
              try {
                playerObj = runner.GetPlayerObject(lclplayer);
              } catch {
                playerObj = null;
              }
            ...
            if (currentViewWidth >= WINDOW_MIN_W + 10) {
              ...
              if (GUI.Button(statsleftrect, isWide ? "<< Stats" : "<<", s_buttonStyle.Value)) {
                var stats = FusionStats.Create(runner: runner, screenLayout: FusionStats.DefaultLayouts.Left);
              ...
              if (GUI.Button(statsrghtrect, isWide ? "Stats >>" : ">>", s_buttonStyle.Value)) {
                var stats = FusionStats.Create(runner: runner, screenLayout: FusionStats.DefaultLayouts.Right);
                EditorGUIUtility.PingObject(stats.gameObject);
                Selection.activeObject = stats.gameObject;
              }
            }
        ...
        if (NetworkProjectConfig.Global.PeerMode == NetworkProjectConfig.PeerModes.Multiple) {
          ...
          if (runnerWithoutVisibilityEnabledDetected) {
            EditorGUILayout.HelpBox($"Runner visibility not enabled. Add a {nameof(RunnerEnableVisibility)} component to your {nameof(NetworkRunner)} Prefab.", MessageType.Warning);
          } 
        }
      }
