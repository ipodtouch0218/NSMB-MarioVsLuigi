...
public class PlayerController : FreezableEntity, IPlayerInteractable {
    ...
    [Networked] public float JumpBuffer { get; set; }
    //Jumping Animations
    [Networked] private NetworkBool IsSingleJump { get; set; }
    [Networked] public NetworkBool IsDoubleJump { get; set; }
    [Networked] public NetworkBool IsTripleJump { get; set; }
    //Knockback
    ...
    private TickTimer GroundpoundInputBuffer { get; set; }
    //Spinner
    ...
    public float jumpLandingTimer, landing, koyoteTime, pickupTimer, powerupFlash;
    ...
    public override void Spawned() {
        ...
        Debug.Log(Runner.GetLocalPlayerData().GetNickname() + " - " + Runner.GetLocalPlayerData().PlayerId);
    }
    ...
    private bool UpdateInputs() {
        var temp = currentInputs;
        if (GetInput(out PlayerNetworkInput newInput)) {
    ...
    public override void FixedUpdateNetwork() {
        ...
        if (IsDead) {
            ...
            if (previousOnGround && !IsOnGround)
    ...
    private void CheckForPowerupActions() {
        ...
        if (currentInputs.buttons.WasPressed(previousInputs.buttons, PlayerControls.PowerupAction)
            || (currentInputs.buttons.WasPressed(previousInputs.buttons, PlayerControls.SprintPowerupAction)
    ...
    #region -- COLLISIONS --
    private static readonly Vector2[] checkDirections = { Vector2.down, Vector2.left, Vector2.right, Vector2.up };
    private readonly HashSet<RaycastHit2D> groundContacts = new(16);
    private void HandleGroundCollision() {
        ...
        float dist = Physics2D.defaultContactOffset * 2f;

        foreach (BoxCollider2D hitbox in hitboxes) {

            Vector2 size = hitbox.size * transform.localScale;
            Vector2 center = body.position + (hitbox.offset * transform.localScale);

            for (int i = 0; i < checkDirections.Length; i++) {
                Vector2 dir = checkDirections[i] * 0.5f;
                Vector2 off = dir * size;
                Vector2 off2 = new Vector2(Mathf.Abs(dir.y), Mathf.Abs(dir.x)) * size;

                //left hit
                groundContacts.Add(Runner.GetPhysicsScene2D().Raycast(center + off - off2, dir, dist, Layers.MaskOnlyGround));
                //right hit
                groundContacts.Add(Runner.GetPhysicsScene2D().Raycast(center + off + off2, dir, dist, Layers.MaskOnlyGround));
            }
        }

        foreach (RaycastHit2D contact in groundContacts) {
    ...
    private void CheckForEntityCollision() {
        ...
        for (int i = 0; i < collisions; i++) {
            ...
            if (collidedObject.GetComponentInParent<IPlayerInteractable>() is IPlayerInteractable interactable) {
                ...
                //don't interact with temporarily invincible players
                if (interactable is PlayerController player && !player.IsDamageable)
                    continue;
    ...
    private void HandleJumping(bool jumpPressed, bool forceJump) {
        if (IsInKnockback || IsDrilling || (State == Enums.PowerupState.MegaMushroom && IsSingleJump))
            return;
        ...
        if (bounce || (forceJump && (IsOnGround || (koyoteTime < 0.07f && !IsPropellerFlying)) && !startedSliding)) {
            ...
            if (!bounce && onSpinner && IsOnGround && !HeldEntity) {
                if (Runner.IsForward) {
            ...
            if (canSpecialJump && IsSingleJump) {
                //Double jump
                IsSingleJump = false;
                IsDoubleJump = true;
                IsTripleJump = false;
            ...
            } else if (canSpecialJump && IsDoubleJump) {
                //Triple Jump
                IsSingleJump = false;
                IsDoubleJump = false;
                IsTripleJump = true;
    ...
    public void UpdateHitbox() {
        bool crouchHitbox = State != Enums.PowerupState.MiniMushroom && pipeEntering == null && ((IsCrouching && !IsGroundpounding) || IsInShell || IsSliding);
    ...
    public Vector2 GetHitboxSize(bool crouching) {
        ...
        if (State <= Enums.PowerupState.Small || (IsStarmanInvincible && !IsOnGround && !crouching && !IsSliding && !IsSpinnerFlying && !IsPropellerFlying) || IsGroundpounding) {
    ...
    private void HandleWalkingRunning(bool left, bool right) {
        ...
        if (IsGroundpounding || IsInKnockback || pipeEntering || jumpLandingTimer > 0 || !(WallJumpTimer.ExpiredOrNotRunning(Runner) || IsOnGround || body.velocity.y < 0))
            return;
        ...
        if ((left ^ right) && (!IsCrouching || (IsCrouching && !IsOnGround && State != Enums.PowerupState.BlueShell)) && !IsInKnockback && !IsSliding) {
            ...
            float newX = body.velocity.x + (acc / Runner.Simulation.Config.TickRate) * direction;
        ...
        IsInShell |= State == Enums.PowerupState.BlueShell && !IsSliding && IsOnGround && functionallyRunning && !HeldEntity && Mathf.Abs(body.velocity.x) >= SPEED_STAGE_MAX[RUN_STAGE] * 0.9f;
    ...
    private void HandleMovement(float delta) {
        ...
        if (currentInputs.buttons.IsSet(PlayerControls.PowerupAction) && IsPropellerFlying && !IsDrilling && body.velocity.y < -0.1f && (PropellerSpinTimer.RemainingTime(Runner) ?? 0f) < propellerSpinTime / 4f) {
        ...
        if (down && !IsGroundpounding && !alreadyGroundpounded) {
            HandleGroundpoundStart(left, right);
        } else {
            GroundpoundInputBuffer = TickTimer.None;
        }
        HandleGroundpound();
    ...
    private void HandleGroundpoundStart(bool left, bool right) {
        ...
        if (!GroundpoundInputBuffer.Expired(Runner)) {
            if (!GroundpoundInputBuffer.IsActive(Runner)) {
                GroundpoundInputBuffer = TickTimer.CreateFromSeconds(Runner, 0.08f);
            }
            return;
        } else {
            GroundpoundInputBuffer = TickTimer.None;
    ...
    public bool CanPickup() {
        return State != Enums.PowerupState.MiniMushroom && !IsSkidding && !IsTurnaround && !HeldEntity && currentInputs.buttons.IsSet(PlayerControls.Sprint) && !IsPropellerFlying && !IsSpinnerFlying && !IsCrouching && !IsDead && !WallSlideLeft && !WallSlideRight && !IsDoubleJump && !IsTripleJump && !IsGroundpounding;
    }
