...
namespace NSMB.UI.MainMenu {
    public class RoomIcon : MonoBehaviour {
        ...
        public void UpdateUI(SessionInfo newSession) {
            ...
            if (timer > 0)
                symbols += "<sprite name=room_timer>" + Utils.Utils.GetSymbolString(timer.ToString(), Utils.Utils.smallSymbols);

            if (lives > 0)
                symbols += "<sprite name=room_lives>" + Utils.Utils.GetSymbolString(lives.ToString(), Utils.Utils.smallSymbols);

            symbols += "<sprite name=room_stars>" + Utils.Utils.GetSymbolString(stars.ToString(), Utils.Utils.smallSymbols);
            symbols += "<sprite name=room_coins>" + Utils.Utils.GetSymbolString(coins.ToString(), Utils.Utils.smallSymbols);
